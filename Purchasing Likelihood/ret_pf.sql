SELECT
  CAST(a.sale_year AS INT64)-CAST(a.aq_year AS INT64) AS ret_years
  ,a.aq_freq AS aq_freq
  ,SUM(customer_count_unique) AS customers_count
  ,SUM(a.sales_amount) AS sales_amount
  ,ROUND(AVG(a.avg_sale), 2) AS avg_sale
  ,SUM(aq_custom_number) AS aq_customer_count
  ,ROUND(AVG(likelihood), 3) AS likelihood
 FROM
...
ORDER BY aq_year,sale_year,aq_freq) a

GROUP BY ret_years,a.aq_freq
ORDER BY ret_years, a.aq_freq